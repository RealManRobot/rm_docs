...
| 3    | 工作坐标系区域：包括开启/结束绘制轨迹、重置视角、位置姿态以及复制位姿信息和复制关节信息。  |
...
点击配置按钮，可进入示教器配置页面，支持进行基础配置、安全配置、Modbus配置、力传感器配置以及查看示教器信息和升级等操作，具体操作请参考[机械臂配置](../setting/index.md)。

![alt text](image-5.png)

## 工作坐标系选择

用户可以选择工作坐标系对机器人运动状态进行控制。在示教器界面选择基坐标系（Base）控制机器人，机器人将会以如图所示的坐标系方向进行移动。用户也可以根据实际项目需求，创建自己所需的工作坐标系作为机器人移动的参考方向，假设work1为用户自己设定的工作坐标系（[设定机器人工作坐标系的方法](../onlineCode/index.md#352411)），设定完成后可进行工作坐标系切换。
...
工具坐标系可通过下拉菜单选择，默认显示的位姿目标是法兰盘中心，用户自己可以设定工具坐标系，假设tool1为用户自己设置的工具坐标系（[设定机器人工具坐标系的方法](../onlineCode/index.md#352412)），设定完成后可进行工具坐标系切换。
...
初始位置：长按`初始姿态`按钮可使机器人回到初始位置。用户可以通过示教器界面来任意设定机器人初始位置（详见[初始位置设定方法](../onlineCode/index.md#352413)）。松开按钮运动停止。
...
X、Y、Z表示工具法兰中心点（选定的工具坐标系）在选定坐标系（基坐标系、末端坐标系、用户自定义坐标系）下的坐标值，RX、RY、RZ 表示相对于选定坐标系旋转的弧度值。<br>
...
<center>图形化编程区域</center>

## 菜单栏选项

点击菜单栏的选项名称可以切换到对应名称面板，方便用户操作。选中的面板呈现灰色背景，具体操作请参考[示教器操作指导](../onlineCode/index.md)。
