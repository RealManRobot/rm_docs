# <p class="hidden">C、C++: </p>机械臂连接配置`ArmRobotic`
...
## <div id = '6235'>初始化线程模式`rm_init()`</div>
...
*可以跳转 [rm_thread_mode_e](../../type/index.md#rm_thread_mode_e线程模式) 查阅枚举类型*
...
| :--- | :--- | :--- |:---|
|   0  |    `int`    |    成功    |-|
|  -1  |    `int`    |    创建线程失败。查看日志以获取具体错误    |通过日志获取线程创建失败时详细的错误信息：<br> Windows创建线程发生错误时，会通过调用`GetLastError`函数获取到具体的错误代码，可在Windows的头文件`<windows.h>`中查看其定义。<br> Linux创建线程发生错误时，会返回`pthread_create`返回值，可在`<pthread.h>`中查看其返回值定义。|
...
创建成功后，返回机械臂控制[rm_robot_handle](../../struct/robotHandle/index.md)句柄id，连接成功id大于0，连接失败返回-1，达到最大连接数5创建失败返回空。
...
## <div id = '6237'>删除指定机械臂实例`rm_delete_robot_arm()`</div>
...
## <div id = '6309'>设置机械臂急停状态`rm_set_arm_emergency_stop()`</div>
...
## <div id = '6239'>获取机械臂基本信息`rm_get_robot_info()`</div>
...
*可以跳转[rm_robot_handle](../../struct/robotHandle/index.md)和[rm_robot_info_t](../../struct/robotInfo/index.md)查阅结构体详细描述。*
...
*这里使用了机械臂事件回调函数`rm_event_callback_ptr`。<br>方法原型为：`typedef void(* rm_event_callback_ptr) (rm_event_push_data_t data)`。<br>跳转[rm_realtime_arm_joint_state_t](../../struct/realtimeArmJointState/index.md)查阅结构体详情。*
