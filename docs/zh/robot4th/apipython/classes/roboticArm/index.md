...
## <div id = '6236'>初始化线程模式`__init__()`</div>
...
*可以跳转[枚举类型说明](../../type/index.md)查阅`rm_thread_mode_e`枚举详细描述。*

- **参数说明:**

|   参数    |   类型    |   说明    |
| :--- | :--- | :--- |
|   `mode`  |    `rm_thread_mode_e`    |    RM_SINGLE_MODE_E：单线程模式，单线程非阻塞等待数据返回；RM_DUAL_MODE_E：双线程模式，增加接收线程监测队列中的数据； RM_TRIPLE_MODE_E：三线程模式，在双线程模式基础上增加线程监测UDP接口数据。    |
...
## <div id = '6238'>删除指定机械臂实例`rm_delete_robot_arm()`</div>
...
## <div id = '6310'>设置机械臂急停状态`rm_set_arm_emergency_stop()`</div>
...
## <div id = '6240'>获取机械臂基本信息`rm_get_robot_info()`</div>
...
2. 返回当前工具坐标系字典：
    ...
    *可以跳转[rm_robot_info_t](../../struct/robotInfo/index.md)查阅结构体详细描述。*
