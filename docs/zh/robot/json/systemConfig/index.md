...
Product_version：机械臂型号，仅第三代控制器支持。
...
| algorithm_info | `string` |算法库信息，其中version代表版本号。|
| ctrl_info | `string` |ctrl层软件信息，其中build_time代表编译时间，version代表版本号。|
| dynamic_info | `string` |动力学版本，其中model_version代表动力学模型版本号。|
| plan_info | `string` |plan层软件信息，其中build_time代表编译时间，version代表版本号。|
...
### 参数保存（第二代控制器）`save_device_info_all`
