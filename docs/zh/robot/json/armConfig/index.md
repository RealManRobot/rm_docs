...
本命令集用于配置机械臂末端的线速度、线加速度、角速度、角加速度、碰撞等级的设置与查询、初始化机械臂参数、设置、恢复和查询机械臂 DH 参数、重设机关节零位补偿等。<br>
机械臂末端参数设置完毕后，第二代控制器需要调用[参数保存（第二代控制器）](../systemConfig/index.md#参数保存第控制器save_device_info_all)手动保存到控制器 Flash，否则断电之后不会保存。
...
### 设置静止状态碰撞检测开关`set_collision_detection`

此接口可使能机械臂在静止状态下的碰撞检测功能。

- **输入参数**

| 参数                  | 类型     | 说明                     |
| :-------------------- | :------- | :----------------------- |
| `set_collision_detection` | `string` | 设置静止状态碰撞检测开关。 |
| `mode`     | `int`    | 0：关闭静止状态碰撞检测功能；1：开启静止状态碰撞检测功能。  |
...
### 设置避奇异`set_avoid_singularity_mode`
...
| `mode`     | `int`    | 0-表示关闭奇异规避；<br>1-表示采用速度优先模式进行奇异规避（只支持6自由度），本模式下轨迹运行过程中，机械臂将在奇异点附近通过改变部分关节姿态避开奇异点，保持运动速度，轨迹精度在奇异点附近会有一定降低。  |
...
- 请参考对应系列机械臂的DH参数表进行设置，其中参数值为0的参数，无法修改，详细参数表请参考[参数说明](../../robotParameter/RM65OntologyParameters/index.md)。
...
### 查询静止状态碰撞检测开关`get_collision_detection`

- **输入参数**

| 参数                  | 类型     | 说明               |
| :-------------------- | :------- | :----------------- |
| `get_collision_detection` | `string` | 查询静止状态碰撞检测开关。 |

- **输出参数**

| 参数              | 类型  | 说明              |
| :---------------- | :---- | :---------------- |
| `collision_stage` | `int` | 0：关闭静止状态碰撞检测功能；1：开启静止状态碰撞检测功能。 |
...
### 查询避奇异模式`get_avoid_singularity_mode`
