#  <p class="hidden">Demo演示(C、C++)：</p>控制器IO端口使用示例
...
result = rm_set_IO_mode(robot_handle, 1, io_1_config);  // 将IO模式设置为输入启动功能复用模式

rm_io_config_t io_2_config = {0};
io_1_config.io_mode = 3;
result = rm_set_IO_mode(robot_handle, 2, io_2_config);  // 将IO模式设置为输入暂停功能复用模式

rm_io_config_t io_3_config = {0};
io_1_config.io_mode = 4;
result = rm_set_IO_mode(robot_handle, 3, io_3_config);  // 将IO模式设置为输入继续功能复用模式

rm_io_config_t io_4_config = {0};
io_1_config.io_mode = 5;
result = rm_set_IO_mode(robot_handle, 4, io_4_config);  // 将IO模式设置为输入急停功能复用模式
```

**C++**  

```C
result = rm_set_IO_mode(robot_handle, 1, 2);  // 将IO模式设置为输入启动功能复用模式
result = rm_set_IO_mode(robot_handle, 2, 3);  // 将IO模式设置为输入暂停功能复用模式
result = rm_set_IO_mode(robot_handle, 3, 4);  // 将IO模式设置为输入继续功能复用模式
result = rm_set_IO_mode(robot_handle, 4, 5);  // 将IO模式设置为输入急停功能复用模式
```
...
数字量输入输出的电压值由接入的参考电压决定，机械臂16芯扩展接口线缆仅提供了12V和24V电源电压。数字量输入输出若需要输出其他电压值，则需要从外部对OUT_P_OUT+、PUT_P_GND引脚引入参考电压。详细接口定义请参考[16芯航插线缆](../../../quickUseManual/interfaceDescriptionArm/index.md#33111)。

### 末端IO接口图

IO复用功能通过程序指令进行切换。出厂时默认引脚3和引脚4为数字输入通道（DI1和DI2），引脚6的电源输出为0V（可通过程序设置）。详细接口定义请参考[6芯航插线缆](../../../quickUseManual/interfaceDescriptionArm/index.md#33112)。
