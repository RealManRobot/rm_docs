...
设定网络名称及密码，网络名称和密码不得包含中文字符，网络密码设定为8-10位密码。点击`设置`按钮，机器人控制器发出“滴”提示音，界面弹窗提示设置成功且自动下载保存网络名称及密码，重启控制器，模式切换完成。
...
点击`确定`按钮，机器人控制器发出“滴滴”提示音，模式切换完成。控制电脑与机器人连接到同一个路由器。
...
本地网络设置：示教器启动时默认控制器的IP地址为192.168.1.18，端口号为8080（机械臂IP修改方式见[设置机械臂IP](../../teachingPendant/setting/index.md#设置机械臂ip)）。
...
2. 如果是有线连接机器人，点击`以太网`--\>`更改适配器选项`.

![本地连接示意图](../../teachingPendant/doc/image178.png)

<center>本地连接示意图</center>

3. 双击“以太网”并打开，并且在弹出的窗口中点击`属性`按钮。

![属性示意图](../../teachingPendant/doc/image179.png)

<center>属性示意图</center>

4. 在弹出的对话框中，第一步选中“Internet协议版本4”，第二步点击`属性`按钮。
...
在示教器IP模式设置栏中，可直接设置机械臂IP，可在目标IP文本框中输入IP，如“192.168.1.19”，输入完毕之后点击设置按钮，可直接设置机械臂IP，此IP设置结束，此设置需重启机械臂生效。如需恢复“192.168.1.18”IP地址，可点击`设置`按钮，机械臂IP即可恢复“192.168.1.18”IP地址，此设置需重启机械臂生效。（如修改过机械臂IP，则登录示教器界面需输入更新过后的IP）
...
机器人配置界面包括安全配置、工具标定、工作坐标系标定、初始姿态设置、安装信息、力传感器配置、力传感器数据图、版本信息、IO设置等。
...
设置零位：关节在掉使能状态下，点击`设零位`按钮后，将指定关节当前位置设置为零位，设置完成后该关节仍处于掉使能状态。点击`上使能`按钮，重新给该关节上使能，才可以控制关节运动。
...
::: warning 注意
用户在设置碰撞防护等级时，应该合理选择，等级越高，机器人检测到误碰撞的概率就越高，如无特殊需求，建议用户可将碰撞等级设置在4左右；另外，如果机器人末端改变工具和负载，需在**工具坐标系标定**时设定工具的质量和质心参数（[工具坐标系标定](../../teachingPendant/setting/index.md#工具坐标系标定)过程参照）以匹配正确的动力学模型。
:::
...
标定完成6个点之后，点击`下一步`，在弹出的“新建工具坐标系”对话框中，点击`开始辨识`，并在弹出的提示框中，点击`确认辨识`，开始根据6个标定点自动计算当前工具信息，计算完成后，点击`添加`，回传给示教器，在工具表格中显示。另外，由于机器人控制器容量有限，只能保存10个工具坐标系，因此在标定前请确认不会超过限制。
...
| 5    | 保持4的姿态，从点4沿工具坐标系X轴正方向移动到第5点，该位置与点4的距离尽可能大于10cm ![5](../../teachingPendant/doc/image203.png) |
| 6    | 保持4的姿态，从点4沿工具坐标系Z轴负方向移动到第6点，该位置与点4的距离尽可能大于10cm ![6](../../teachingPendant/doc/image204.png) |
...
- 区域1显示目前机器人建立的初始位姿关节角度；
- 区域2为用户手动设置虚拟机械臂，将6个关节的角度设定好后，点击设置按钮，即可将设定好的位姿关节角度发送给机器人保存，设置成功后在区域1中显示；
- 区域3为自动设置，将机器人6个关节的当前角度设置为初始位姿关节角度，设置成功后在区域1中显示。
...
力位混合控制参数设置，支持设置力方向惯性、力方向阻尼、力方向刚度、力矩方向惯性、力矩方向阻尼和力矩方向刚度，参数修改完成后点击`设置`完成设置，如需恢复初始设置，请点击`重置`。

![力位混合控制参数设置](../../teachingPendant/doc/image231.png)

<center>力位混合控制参数设置</center>
...
![一维力手动标定](../../teachingPendant/doc/image218.png)
...
连接机器人后，版本信息界面可以查看机械臂型号、示教器版本、控制器软件版本、算法版本、动力学模型版本、关节软件版本和末端接口版本。
...
- **控制器电源输出:** 该部分可配置控制器对外的输出电源，可配置为0V，12V和24V。
- **数字输入/输出:** 该部分显示数字IO的状态，用户根据实际需要设定为输入、输出、输入开始功能复用模式、输入暂停功能复用模式、输入继续功能复用模式、输入急停功能复用模式、输入进入电流环拖动复用模式、输入外部轴最大限位复用模式、输入外部轴最小限位复用模式、输入初始位姿功能复用模式、输入碰撞功能复用模式、实时调速功能复用模式等相应功能。
- **设定为输入接口时：** 灰色圆圈代表低电平，绿色圆圈代表高电平。
- **设定为输出接口时：** 按下按钮颜色变为绿色，对应通道设置输出状态为高；按钮弹起变为灰色，代表对应通道输出为低。
...
- 虚拟墙是创建一个虚拟墙壁，将机械臂拖动示教的运动范围限制在墙内，虚拟墙作用于机械臂整体时，当拖动机械臂过程中，机械臂的某一部分碰到墙体并要超出墙体限制时，会感受到阻力，并且无法向冲出墙体的方向运动。
- 虚拟墙功能目前支持长方体和球体两种形状，实现对拖动范围进行精确限制。在电流环拖动示教与力控拖动示教两种示教模式下，用户可以借助虚拟墙功能，确保机械臂的拖动操作不会超出预设的范围。
- 虚拟墙功能目前仅支持电流环拖动示教与力控拖动示教两种示教模式。在其他操作模式下，此功能将自动失效。因此，请确保在正确的操作模式下使用虚拟墙功能，以充分发挥其限制拖动范围的作用。

**设置虚拟墙**

1. 打开示教器，并选择`配置->机械臂配置->虚拟墙`，进入虚拟墙页面。
2. 参照下表设置相关参数。<br>
...
|虚拟墙形状|目前支持长方体和球体。|
|选择参数|支持选择已保存的虚拟墙参数。|
|设置参数|x，y，z均基于世界坐标系，单位0.001m，最大范围是-5~+5（m）。|
|拖动区域|默认为墙内，不支持修改。|
|作用部位|支持选择虚拟墙作用于整臂或者末端。|

![虚拟墙参数设置](../../teachingPendant/doc/image276.png)

<center>虚拟墙参数设置</center>

3. 设置完成后点击`新建参数`，并在弹出的提示框中输入新建虚拟墙的名称后，点击`保存`，完成虚拟墙新建。<br>
当`选择参数`选择了已保存的虚拟墙时，此时需点击`更新参数`，更新已保存虚拟墙的参数。<br>
保存后的虚拟墙可在`数据管理`中的`几何模型参数`查看。

![新建虚拟墙](../../teachingPendant/doc/image278.png)

<center>新建虚拟墙</center>

**开启虚拟墙**

1. 在虚拟墙页面，点击`开启`完成开启虚拟墙。
2. 开启后虚拟墙后，使能状态将会显示为`使能`，同时图标变为绿色，`开启`按钮变为绿色`关闭`，在模型中可看到设置的虚拟墙变为绿色。

![虚拟墙开启](../../teachingPendant/doc/image277.png)

<center>虚拟墙开启</center>

3. 虚拟墙设置完成后，需进入`扩展-末端控制`，在电流环拖动中进行拖动示教，此时虚拟墙有效，拖动机械臂时，可以感受到机械臂的运动被限制在墙内。
...
::: warning 注意
如果在已开启电子围栏情况下，重新设置了安装角度，需根据当前安装角度重新配置电子围栏。
:::
