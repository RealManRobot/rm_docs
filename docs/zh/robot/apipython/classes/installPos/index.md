# <p class="hidden">Python: </p>系统安装方式配置`InstallPos`
...
2. dict[str,any]: 包含以下键值的字典:
    ...
    |   参数    |   类型    |   说明    |
    | :--- | :--- | :--- |
    | `version` |    list[int]  |   获取到的各关节软件版本号数组。    |
    | `joints_v` |    list[str]   |   预留参数，第三代控制器不适用。   |
...
2. dict[str,any]: 包含以下键值的字典：
    ...
    |   参数    |   类型    |   说明    |
    | :--- | :--- | :--- |
    |  `version` |    list[int]  |    获取到的末端接口板软件版本号。    |
    | `tool_v` |    list[str]   |    预留参数，第三代控制器不适用。   |
