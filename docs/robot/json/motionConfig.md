...
- 固定模式（0）：该方向处于锁轴状态，不会发生运动。
- 浮动模式（1）：该方向表现为滑块，受到外力后末端将沿着该轴运动，撤掉外力后该方向将保持位置/姿态不变。
- 弹簧模式（2）：该方向表现为弹簧，受到外力后末端将沿着该轴运动，撤掉外力后该方向位置/姿态将自动恢复到受力前状态。
- 运动模式（3）：该方向受MOV指令轨迹规划控制。
- 力跟踪模式（4）：该方向维持设定的期望力/力矩，例如z方向的期望力设为10N，那么控制器将会维持该方向的力为10N，即恒力控制。
- 力跟踪模式+姿态自适应模式（8）：该方向维持设定的期望力/力矩，并且力控参考坐标系的Rx、Ry和Rz方向的姿态可以根据接触环境的变化自动调整。该模式只能在力控参考坐标系的Z方向开启，并且力控参考坐标系必须为工具坐标系。开启该模式后，力控参考坐标系的Rx、Ry和Rz方向设置的任何模式都不生效。
