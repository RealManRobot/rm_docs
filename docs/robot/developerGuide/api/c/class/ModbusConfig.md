...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    线圈读取参数结构体，该指令最多一次性支持读 8 个线圈数据，即返回的数据不会超过一个字节。    |
|   data  |    `int`    |    返回线圈状态，数据类型：int8。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    离散量输入读取参数结构体，该指令最多一次性支持读 8 个离散量数据，即返回的数据不会超过一个字节。    |
|   data  |    `int`    |    返回离散量，数据类型：int8。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    保持寄存器数据读取参数结构体，该指令每次只能读 1 个寄存器，即 2 个字节的数据，不可一次性读取多个寄存器数据，该结构体成员num无需设置。    |
|   data  |    `int`    |    返回寄存器数据，数据类型：int16。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    输入寄存器数据读取参数结构体，该指令每次只能读 1 个寄存器，即 2 个字节的数据，不可一次性读取多个寄存器数据，该结构体成员num无需设置。    |
|   data  |    `int`    |    返回寄存器数据，数据类型：int16。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    单圈数据写入参数结构体，该结构体成员num无需设置。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    多个寄存器数据写入参数结构体。其中寄存器每次写的数量不超过10个，即该结构体成员num<=10。    |
|   data  |    `int`    |    要写入寄存器的数据数组，类型：byte。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    多圈数据写入参数结构体。每次写的数量不超过 160 个，即该结构体成员num<=160。    |
|   data  |    `int`    |    要写入线圈的数据数组，类型：byte。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    多圈数据读取参数结构体，要读的线圈的数量 8< num <= 120，该指令最多一次性支持读 120 个线圈数据， 即 15 个 byte。    |
|   data  |    `int`    |    返回线圈状态，数据类型：int8。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    多个保存寄存器读取参数结构体，要读的寄存器的数量 2 < num < 13，该指令最多一次性支持读 12 个寄存器数据，即 24 个byte。    |
|   data  |    `int`    |    返回寄存器数据，数据类型：int8。    |
...
| :--- | :--- | :--- |
|   handle  |    `/`    |    机械臂句柄。    |
|   params  |    `/`    |    多个输入寄存器读取参数结构体，要读的寄存器的数量 2 < num < 13，该指令最多一次性支持读 12 个寄存器数据， 即 24 个 byte。    |
|   data  |    `int`    |    返回寄存器数据，数据类型：int8。    |
