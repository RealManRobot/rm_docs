...
| :--- | :--- | :--- |
|   handle  |    用户输入    |    机械臂句柄。    |
|  joint_num  |    用户输入    |    关节序号，1~7。    |
|  step  |    用户输入    |    步进的角度。    |
|  v  |    用户输入    |    度比例1~100，即规划速度和加速度占关节最大线转速和加速度的百分比。    |
...
|   handle  |    用户输入    |    机械臂句柄。    |
|  type  |    用户输入    |    示教类型。    |
|  step  |    用户输入    |    步进的距离，单位m，精确到0.001mm。    |
|  v  |    用户输入    |    度比例1~100，即规划速度和加速度占关节最大线转速和加速度的百分比。    |
|  block  |    用户输入    |    阻塞设置：<br>多线程模式：0，非阻塞模式，发送指令后立即返回；1，阻塞模式，等待机械臂到达目标位置或规划失败后返回。<br>单线程模式：0，非阻塞模式；其他值，阻塞模式并设置超时时间，根据运动时间设置，单位为秒。    |
...
|   handle  |    用户输入    |    机械臂句柄。    |
|  type  |    用户输入    |    示教类型。    |
|  step  |    用户输入    |    步进的弧度，单位rad，精确到0.001rad。    |
|  v  |    用户输入    |    度比例1~100，即规划速度和加速度占关节最大线转速和加速度的百分比。    |
|  block  |    用户输入    |    阻塞设置：<br>多线程模式：0，非阻塞模式，发送指令后立即返回；1，阻塞模式，等待机械臂到达目标位置或规划失败后返回。<br>单线程模式：0，非阻塞模式；其他值，阻塞模式并设置超时时间，根据运动时间设置，单位为秒。    |
...
| :--- | :--- | :--- |
|   handle  |    用户输入    |    机械臂句柄。    |
|  frame_type  |    用户输入    |    0: 工作坐标系运动, 1: 工具坐标系运动。    |
...
| :--- | :--- | :--- |
|   handle  |    用户输入    |    机械臂句柄。    |
|  joint_num  |    用户输入    |    示教关节的序号，1~7。    |
|  direction  |    用户输入    |    示教方向，0-负方向，1-正方向。    |
