...
| :--- | :--- | :--- |
|   handle  |    用户输入    |    机械臂句柄。    |
|  joint  |    用户输入    |    目标关节1~7角度数组。    |
|  v  |    用户输入    |    速度比例1~100，即规划速度和加速度占关节最大线转速和加速度的百分比。    |
|  r  |    用户输入    |    轨迹交融半径，目前默认0。    |
|  trajectory_connect  |    用户输入    |    轨迹连接标志:<br>0：立即规划并执行轨迹，不与后续轨迹连接；<br>1：将当前轨迹与下一条轨迹一起规划，但不立即执行,阻塞模式下，即使发送成功也会立即返回。    |
|  block  |    用户输入    |    阻塞设置：<br>多线程模式：0，非阻塞模式，发送指令后立即返回；1，阻塞模式，等待机械臂到达目标位置或规划失败后返回。<br>单线程模式：0，非阻塞模式；其他值，阻塞模式并设置超时时间，根据运动时间设置，单位为秒。    |
...
| :--- | :--- | :--- |
|   handle  |    用户输入    |    机械臂句柄。    |
|  pose_via  |    用户输入    |    中间点位姿，位置单位：米，姿态单位：弧度。    |
|  pose_to  |    用户输入    |    终点位姿，位置单位：米，姿态单位：弧度。    |
|  v  |    用户输入    |    速度比例1~100，即规划速度和加速度占关节最大角速度和角加速度的百分比。    |
|  r  |    用户输入    |    轨迹交融半径，目前默认0。    |
|  loop  |    用户输入    |    规划圈数。    |
|  trajectory_connect  |    用户输入    |    轨迹连接标志:<br>0：立即规划并执行轨迹，不与后续轨迹连接；<br>1：将当前轨迹与下一条轨迹一起规划，但不立即执行,阻塞模式下，即使发送成功也会立即返回。    |
|  block  |    用户输入    |    阻塞设置：<br>多线程模式：0，非阻塞模式，发送指令后立即返回；1，阻塞模式，等待机械臂到达目标位置或规划失败后返回。<br>单线程模式：0，非阻塞模式；其他值，阻塞模式并设置超时时间，根据运动时间设置，单位为秒。    |
...
| :--- | :--- | :--- |
|   handle  |    用户输入    |    机械臂句柄。    |
|  joint  |    用户输入    |    关节1~7目标角度数组,单位：°。    |
|  follow  |    用户输入    |    true-高跟随，false-低跟随。若使用高跟随，透传周期要求不超过10ms。    |
|  expand  |    用户输入    |    如果存在通用扩展轴，并需要进行透传，可使用该参数进行透传发送。    |
