...
char* get_absolute_path(const char* relative_path) {  
...
#ifdef _WIN32  
    // Windows 
    DWORD size = GetFullPathNameA(relative_path, 0, NULL, NULL);  
    if (size == 0) {  
        return NULL;  
    }  
      
    absolute_path = (char*)malloc(size * sizeof(char));  
    if (absolute_path == NULL) {  
        return NULL;  
    }  
      
    if (GetFullPathNameA(relative_path, size, absolute_path, NULL) == 0) {  
        free(absolute_path);  
        return NULL;  
    }  
#else  
    ...
    absolute_path = strdup(resolved_path);  
    free(resolved_path);   
#endif  
