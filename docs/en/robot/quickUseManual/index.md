...
![Product components](product.png)

**Explanation of the Electrical Indications**  

![Comparison between Gen 2 and Gen 3 controllers](compare.png)

The left one in the figure above is Gen 3 controller. Compared with Gen 2 controller on the right, the status indicator light is added and a rarely used network port is removed.
...
| 3    | Multiplexing I/O interface    |  Lead out the RS485 and I/O interfaces, etc. <br> For example, the external emergency stop button box. For detailed information, please refer to the [External Emergency Stop Button Box](../../blog/arm/ExternalEmergencyStopButtonBox/index.md).|
| 4    | WiFi      | For wireless communication, 100 Mbps Wi-Fi. |
| 5    | USB port      | An extension interface, for connecting the bluetooth handle receiver |
| 6    | Ethernet port    | Gigabit wired network interface, Communication network interface. |
| 7    | Interactive RGB indicator | Change among blue, white, green, yellow, and red with the status of robotic arm <br> ① <font color=Blue>Blue</font> indicates controller start and initialization;<br>②White indicates joint start and initialization;<br>③<font color=Green>Green</font> indicates normal operation of robotic arm;<br>④<font color= #FFA500>Yellow</font> indicates the warning of a common fault, which must be handled immediately;<br>⑤<font color=Red>Red</font> indicates a major fault, which must be handled immediately. |
...
5. Press the power switch to start the robotic arm.
   ...
   ② Meanwhile, the indicator light in the upper right corner of the robot controller turns blue, indicating that the controller is starting. <br>
   ③ The indicator light of the controller turns white, indicating that the robot joints are being initialized. <br>
...
The teach pendant carrier and the robotic arm can be connected by wired or wireless means.
...
- Open the teach pendant as follows:<br>
  ...
  (2) Enter the account number "user" and password "123", and click "Login".
  ![alt text](../quickUseManual/operation/666.png)
  (3) Open the robotic arm teaching interface.
  ![alt text](../quickUseManual/operation/777.png)

**Wireless connection:**

Users connect their host computers to the WiFi access point of robotic arm.

- WiFi connection
  The default name format of WiFi access point is as shown in the figure below. The password is "12345678" by default:

  ![alt text](CleanShot.png)

- Open the teach pendant as follows:<br>
  (1) Open a browser, and if the connection is made through the wireless access point, enter the website "192.168.33.80". <br>
  (2) Enter the account number "user" and password "123", and click "Login".
  ![alt text](../quickUseManual/operation/666.png)
  (3) Open the robotic arm teaching interface.
...
Here is just simple description of teaching motion control. For more details, refer to the [Guide to Use of Web Teach Pendant of Realman Robotic Arm](../teachingPendant/armTeching/index.md). For further secondary development, refer to the interface instructions open to users at the website.
