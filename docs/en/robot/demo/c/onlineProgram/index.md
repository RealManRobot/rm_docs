#  <p class="hidden">Demo (C, C++): </p>Online Programming Demo
...
Download `RM_API2` locally via the link: [development package download](https://github.com/RealManRobot/RM_API2.git). Then, navigate to the `RM_API2\Demo\RMDemo_C` directory, where you will find RMDemo_OnlineProgram.
...
2. **Running via linux command line**:
   Navigate to the `RMDemo_OnlineProgram` directory in the terminal, and enter the following command to run the C program:
