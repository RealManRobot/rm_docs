...
If users directly use our packaged API, they can only use C/C++/C#/Python/MATLAB to call it in Linux/Windows environment. If users use the JSON protocol, the flexibility will be much higher. There is no restriction on users' system and programming language, as long as a fixed string can be sent according to the protocol. The JSON protocol is often used in PLC, Android, IOS and other platforms.

### 2. Can the robotic arm be controlled through the Android platform/PLC?

The robotic arm supports the Web teach pendant. It can be accessed by opening a browser after establishing a network connection, and can be accessed in Android, IOS, WIN and LINUX systems.
The robotic arm can be controlled as long as it receives the string of JSON format. The string can be sent and received through the Ethernet port, and RS485. Using an Android device, PLC, single chip microcomputer, etc., users can control the robotic arm by directly sending strings of JSON format. Users can also use MODBUS-RTU/TCP to control the robotic arm.
...
- View the current joint angles in the robotic arm teaching interface;
- In Configuration - Security Config, select the corresponding joint and adjust the joint limits to be greater than the current angle;
- In the robotic arm state interface, clear the joint error and select the corresponding joint, then click the Enable button;
- In the robotic arm teaching interface, control the joint to return within the limits;
- In the robotic arm state interface, select the corresponding joint and click the Disable button;
- In Configuration - Security Config, select the corresponding joint, restore the limits to their initial values, and re-enable.
...
**Pass-through cycle of Gen 4 controllers:** as fast as 5 ms, or 2 ms if the real-timeliness of the upper computer is high enough.
...
### 6. What are the I/O multiplexing functions of Gen 4 controller?
