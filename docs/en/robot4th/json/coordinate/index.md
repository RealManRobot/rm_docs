...
This instruction set is used to configure the tool frame of the robotic arm, including computing the tool frame, inputting the tool frame, changing the current tool frame, deleting the tool frame, and updating the tool frame of the tool frame.

### <div id = '245512'>Automatically compute the tool frame (calibrate reference points) `set_auto_tool_frame`</div>
...
After calibrating 6 reference points successfully by [Automatically compute the tool frame (calibrate reference points)](#245512), run this command to configure the tool frame.
