...
手眼标定通常用于机器人和计算机视觉领域，特别是在需要精确控制机械臂与环境交互的场景中去，手眼标定是将机械臂的末端执行工具（二指夹爪等）和摄像机的坐标系统一起来，解决相机与机械手之间的坐标转换关系，让机械手能精确抓取到相机定位的目标。
...
- 标定板到机械臂末端的变换矩阵（因为标定过程中标定板固定在机械臂末端，标定板到机械臂末端的变化矩阵不变）
$$^{end}_{board}M$$
...
- 使用OpenCV的 `calibrateHandEye` 函数进行手眼标定
  ...
  1. 在compute_in_hand.py中计算**相机**相对于**机械臂末端**的旋转矩阵 **R_cam2end**和平移向量 **T_cam2end**。
  ...
  2. 在compute_to_hand.py中计算**相机**相对于**机械臂基座**的旋转矩阵 **R_cam2base** 和平移向量**T_cam2base**。
...
2. 打开示教器，查看当前机械臂控制器版本
   ...
   ![image-20241025135921401](../developerGuide/doc/handEye_17.png)

3. 查看机械臂控制器版本和当前程序使用的机械臂API版本是否对应

   ![image-20241025134353953](../developerGuide/doc/handEye_18.png)

4. 根据控制器版本和API版本对应关系，选择特定版本的控制器软件或者机械臂API进行升级替换

    - 升级控制器版本方法[web版示教器使用说明](../../robot/download/userManual.md)
...
![image-20241025132928948](../developerGuide/doc/handEye_22.png)

config.yaml里的配置文件如下，有下面四个

1. ROBOT_TYPE：机械臂型号，默认是65 ，对应RM65臂
2. xx:标定板的横向角点数（长边格子数减1），默认为11，例如下图长边12个格子，角点数为11
3. YY:标定板纵向角点数（短边格子数减1），默认为8，例如下图短边9个格子，角点数为8
4. L :  标定板单个方格的实际尺寸（单位：米），默认是0.03
...
- 使出现在相机视野里的**标定板**和**相机镜面**呈现**一定角度**。
  ...
  ![image-20241101112801145](../developerGuide/doc/handEye_21.png)

  下面的拍照姿势**不对**：
  ![image-20241101112816914](../developerGuide/doc/handEye_20.png)
...
移动**机械臂末端旋转轴**，每次旋转的角度尽量的大（大于30°）。确保在三个轴（X、Y、Z）上都有足够的旋转角度变化。（可以先绕着机械臂末端z轴旋转多个角度拍摄多副图片，然后绕着x轴旋转）

![WPS拼图1](../developerGuide/doc/handEye_19.png)
...
**问题原因：** 当前程序API版本和机械臂控制器版本不一致

**解决方案：**

1. 打开示教器，查看当前机械臂控制器版本
  - 示教器：配置-机械臂配置-版本信息

  ![image-20241025135921401](../developerGuide/doc/handEye_17.png)

2. 当前使用机械臂API版本（4.3.3）
- 查看机械臂控制器版本和当前程序使用的机械臂API版本是否对应

  ![image-20241025134353953](../developerGuide/doc/handEye_18.png)

3. 根据控制器版本和API版本对应关系，选择特定版本的控制器软件或者API进行升级替换
  - 升级控制器版本方法[web版示教器使用说明](../../robot/download/userManual.md)
...
![image](../developerGuide/doc/handEye_13.png)
